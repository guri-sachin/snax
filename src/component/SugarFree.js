...
const Sugarfree = () => {
  ...
  const scrollRight = () => {
    const newPosition = scrollPosition + 400;
    setScrollPosition(newPosition > products.length * 400 ? 0 : newPosition); // Auto-loop to the start
  };
...
//         <div
//           ref={containerRef}
//           className="flex overflow-x-auto w-full no-scrollbar scroll-smooth"
//         >
//           {products.map((product) => (
//             <div key={product.id} className="w-full sm:w-1/2 lg:w-1/3 flex-shrink-0" onClick={() => handleProductClick(product)}>
//               <ProductCard
//                 image={product.product_feature_img}
//                 title={product.product_short}
//                 mrp={product.product_price ? product.product_price.split(',')[0] : 'N/A'}
