...
const ProductCard = ({ image, title, price, mrp, save, stocks}) => {
  ...
  return (
    <div
    className={`relative overflow-hidden p-2 rounded-lg transition-all duration-300 mx-4 ${isHovered ? 'bg-gray-100' : ''}`}
    onMouseEnter={handleMouseEnter}
    onMouseLeave={handleMouseLeave}
  >
    {/* Save Badge */}
    <div className="absolute top-2 right-2 bg-red-600 text-white text-xs font-bold py-1 px-2 rounded z-10">
    <p>{stocks == 0 ? save : 'Sold out'}</p>
    </div>
    ...
    <div className="text-center mt-4">
      ...
      <p className="text-xl font-bold text-red-600 mt-1">
        ...
        <span className="text-gray-500 mx-2">&rarr;</span>₹{price}
      </p>
...
const Collection = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const response = await axios.get(`${apiUrl}/trending-products`);
