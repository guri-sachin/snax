...
const TrendingProductsSlider = () => {
  ...
  const handleProductClick = (product) => {
    ...
    SecureStorage.setItem('id',  product.product_id);
     ...
     // Construct the product URL (you can change this based on your route structure)
  const productUrl = `/product/${product.product_slug.replace(/\s+/g, '-')}`;
  ...
  (inclusive of all taxes)
      ...
      <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
        <button className="px-4 py-2 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-all duration-200"   onClick={() => handleProductClick(product)}>
          Get it
        </button>
      </div>
    </div>
