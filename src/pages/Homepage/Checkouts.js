...
const calculateTotalSize = (cart) => {
  let totalSize = 0;

  cart.forEach(item => {
    if (item.quantity && item.size) {
      // Extract numerical value from size string (e.g., "100g" -> 100)
      const sizeInGrams = parseInt(item.size.replace('g', ''), 10);
      
      // Only add if sizeInGrams is a valid number
      if (!isNaN(sizeInGrams)) {
        totalSize += sizeInGrams * item.quantity;
      }
    }
  });

  return totalSize; // Total size in grams
};

// Usage
let totalOrderSize = calculateTotalSize(cart);
console.log(`Total Order Size: ${totalOrderSize} `);

// Add 200 grams (0.20kg) if total size is zero
if (totalOrderSize === 0 || totalOrderSize === 1 ) {
  totalOrderSize = 200; // Add 200 grams
}

const totalOrderSizeKg = (totalOrderSize / 1000).toFixed(2); // Convert to kg and format to 2 decimal places




console.log(`Total Order Size: ${totalOrderSizeKg} grams`);
    ...
    <div className="bg-gray-100 min-h-screen py-10 px-5">
      ...
      {email ? (
        ...
        <p className="text-sm text-gray-600 mt-2">
          We are using this email to create your account. You will use this mail for login.
        </p>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" className="w-full p-3 border border-gray-300 rounded" placeholder="Last name"              onChange={(e) => setLname(e.target.value)}/>
              </div>
            </div>
