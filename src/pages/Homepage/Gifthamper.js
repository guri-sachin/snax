...
const CustomiseGift = () => {
  ...
  const categore = location.state?.category; // Access the category
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        // Make the API call with optional category
        const response = await axios.get(`${apiUrl}Allgifts`, {
          params: { categore: categore || "" }, // Pass categore as query parameter
        });
  ...
  const handleProductClick = (hamper) => {
    ...
    const encryptedId = CryptoJS.AES.encrypt(hamper.id.toString(), 'gurdeep').toString();
  
    // Navigate to the product page with both slug and encrypted ID
    navigate(`/gifts/${hamper.slug.replace(/\s+/g, '-')}/${encodeURIComponent(encryptedId)}`, {
      state: { hamper },
    });
...
*/}
  ...
  {hampers.map((hamper) => {
    ...
    const imageArray = JSON.parse(hamper.img);
    ...
    return (
      ...
      >
        ...
        {/* Card Body */}
        <div className="p-4">
          <h3 className="text-sm font-bold text-gray-800 mb-2">{hamper.title}</h3>
          <div className="flex justify-center items-center gap-2">
           
            <p className="text-sm text-red-600 line-through">MRP: ₹{hamper.mrp}</p>
            <p className="text-lg font-semibold text-gray-900">₹{hamper.price}</p>
