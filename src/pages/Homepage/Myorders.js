...
<div className="bg-white shadow-lg rounded-lg p-6 mt-4 w-full max-w-2xl mx-auto">
  ...
  <div className="border-t border-gray-200 pt-4">
    <ul>
      {order.items.split(',').map((item, idx) => {
        ...
        return (
          <li key={idx} className="flex flex-col sm:flex-row items-center sm:justify-between space-y-4 sm:space-y-0 sm:space-x-4 py-4 border-b border-gray-100">
  ...
  <>
    <p className="text-lg font-bold">Payable Amount: {order.afterdiscount}</p>
    <p className="text-lg font-bold">Total Amount: {order.totalAmount}</p>
  </>
) : (
  <p className="text-lg font-bold">Total Amount: {order.totalAmount}</p>
)}
    ...
    <div className="mt-4">
      <h4 className="text-md font-semibold">Shipping Address:  {order.paymentMethod}</h4>
