...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchProperty = async () => {
      try {
        ...
        // Convert it to a number (remove commas and ensure it's a valid number)
        // const numericPice = parseFloat(firstPice.replace(/,/, ''));
      ...
      <div className="flex overflow-x-scroll md:overflow-y-scroll md:flex-col gap-2 scrollbar-hide md:h-[640px] mt-4 md:mt-0">
        {images.map((image) => (
          <img
            ...
            src={image}
            alt="Thumbnail"
            className="cursor-pointer rounded-lg shadow-sm hover:shadow-lg h-[100px] w-[100px] md:h-[150px] md:w-[150px] md:p-3"
  ...
  MRP ₹{productmrp[selectedButton]}
              ...
              <p className="text-sm text-gray-500 mt-1 pr-2">
                (Incl. of all taxes)
              </p>
            </div>
