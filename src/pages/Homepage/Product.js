...
const ProductDetail = () => {
  ...
  const [id, setId] = useState(null);  // State to store decrypted ID
  ...
  useEffect(() => {
    ...
    const fetchProperty = async () => {
      try {
        ...
        setHSN(fetchedProperty.product_hsn)
  ...
  const handleButtonClick = (buttonIndex, label) => {
    ...
    setSelectedImage(images[buttonIndex]);
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ ...product, size, quantity, price, title, sku, id, hsn});
    Swal.fire({
      title: 'Checkout Now!',
  ...
  const handleBuynow = () => {
    addToCart({ ...product, size, quantity: 1, price, title, sku, id, hsn });
  ...
  <div className="flex flex-col-reverse md:flex-row">
      ...
      <div className="flex overflow-x-scroll md:overflow-y-scroll md:flex-col gap-2 scrollbar-hide md:h-[640px] mt-4 md:mt-0">
        {images.map((image) => (
          <img
            ...
            src={image}
            alt="Thumbnail"
            className="cursor-pointer rounded-lg shadow-sm hover:shadow-lg h-[100px] w-[100px] md:h-[160px] md:w-[160px] md:p-3"
    ...
    {/* Main Image Display */}
    <div
      className="w-full md:w-3/4 relative rounded-lg  shadow-lg mb-4 overflow-hidden"
    ...
    >
      <img
        src={selectedImage}
        alt="Selected"
        className={`w-full h-full object-contain transform transition-transform duration-300 ${zoom ? 'scale-150' : 'scale-100'}`}
        style={{
