...
// Empty dependency array ensures this runs only once
  ...
  const handleButtonClick = (buttonIndex,label) => {
   ...
   setSKU(skus[buttonIndex]);
    console.log("label",label,buttonIndex,sku)
  };
...
console.log("sku",skus,prices)
  useEffect(() => {
    if (product.product_img || product.actualp_editor || product.product_p_pice || product.product_p_price || product.product_price || product.product_points || product.product_sku) {
...
setSKUs(skusall);
  ...
  }, [product.product_img,product.actualp_editor,product.product_p_pice,product.product_p_price,product.product_price, product.product_points || product.product_sku]);
...
generateRandomNumber();
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ ...product, size, quantity, price, title, sku, id }); // Here quantity is set to 1 for a new addition
  };
  ...
  <div className="grid grid-cols-4 gap-2 mx-16">
    ...
    >
      ...
      <div className="absolute top-2 right-2 bg-orange-500 text-white text-sm font-bold px-2 py-1 rounded z-10">
        Save {product.product_p_discount}%
      </div>
    </div>
    {images.map((image) => (
      <img
  ...
  <div className="grid grid-cols-2 gap-4 md:grid-cols-4 mb-6 mt-4">
   ...
   <div className="container mx-auto lg:flex lg:space-x-6 mt-10  border-gray-200 rounded-lg p-4 lg:px-12">
