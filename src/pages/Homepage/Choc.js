...
const CustomiseGift = () => {
  ...
  const [page, setPage] = useState('choclate'); // State to hold the page input
  ...
  const handleProductClick = (product) => {
    ...
    const sortedProducts = hampers.sort((a, b) => {
...
{seoData?.seoKeyword && <meta name="keywords" content={seoData.seoKeyword} />}
   ...
   {/* slider */}
      ...
      <div className="container mx-auto sm:px-4 lg:px-8 md:px-8 ">
  ...
  {/* Display filtered hampers in a responsive grid */}
  <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 lg:gap-6 sm:gap-6 gap-2 md:gap-6  ">
    {hampers.map((hamper) => {
        ...
        >
          ...
          <div className="relative overflow-hidden">
            <img
              ...
              loading="lazy"
              className="w-full lg:h-48 sm:h-20 object-cover transition-opacity duration-300 hover:opacity-0"
            />
            {imageArray && imageArray.length > 0 && (
              <img
                ...
                alt={hamper.title}
                loading="lazy"
                className="absolute inset-0 w-full lg:h-48  sm:h-20 object-cover transition-opacity duration-300 opacity-0 hover:opacity-100"
              />
            )}
          </div>
          ...
          {/* Card Body */}
          <div className="p-2">
            <h3 className="text-sm font-bold text-gray-800 mb-2 text-center">{hamper.product_title}</h3>
            <div className="flex justify-center items-center gap-2">
              <p className="text-sm text-red-600 line-through">MRP: ₹{hamper.product_price}</p>
              <p className="text-lg font-semibold text-gray-900">₹{hamper.gifts_price}</p>
            </div>
            <p className="text-sm text-gray-500 mt-1 text-center">(Incl. of all taxes)</p>
          </div>
        </div>
      );
    })}
